...
const Hero: React.FC<IHero> = ({ t }) => {
  ...
  const startAnimation = (nextItem: number) => {
    ...
    if (nextImage.complete) {
      ...
      nextImage.onload = () => requestAnimationFrame(animate);
    }
  };
  ...
  useEffect(() => {
    ...
    if (canvas && newCanvas && context && currentImage) {
      setCanvasSize(canvas);
      setCanvasSize(newCanvas);
      context.clearRect(0, 0, canvas.width, canvas.height);
        ...
        {Array.from({ length: nbItems }).map((_, index) => {
          ...
          return (
            <button
              ...
              aria-label={
                isActive
                  ? `${t.activeSlideAriaLabel} ${index + 1}`
                  : `${t.slideAriaLabel} ${index + 1}`
              }
            />
          );
        })}
